...
def main():
    ...
    for target_temp in tqdm(temp_range):
        ...
        for i in tqdm(range(nr_rounds)):
            ...
            baseline_outputs = model.generate(input_ids=None, encoder_outputs=encoder_outputs, max_length=300, min_length=100, 
                                            num_beams=1, do_sample=True, temperature=0.15)
            ...
            baseline_sum_output = summarizer.generate(input_ids=None, encoder_outputs=baseline_encoder_outputs, max_length=70, output_hidden_states=True,
                                                    return_dict_in_generate=True, do_sample=True, num_beams=1, temperature=target_temp)
    ...
    plt.title("Cosine Similarity for different Temperatures of LLM #2")
    plt.grid()
    plt.legend()
    plt.show()
    ...
    plt.title("Mututal Information for different Temperatures of LLM #2")
    plt.grid()
    plt.legend()
    plt.show()
