...
def main():
    ...
    for _ in range(nr_rounds):
        input_text_ls.append(masking(original_text, 7/130))
    ...
    for target_temp in tqdm(temp_range):
        ...
        for i in tqdm(range(nr_rounds)):
            ...
            noisy_outputs = model.generate(input_ids=None, encoder_outputs=modified_encoder_outputs, max_length=300, min_length=100, 
                                        num_beams=15, do_sample=True, temperature=target_temp, early_stopping=True)
    ...
    plt.plot(temp_range, cs_list_mean, marker='o')
    # plt.axhline(y=cs_base_orig_mean, color='r', linestyle='--', label='Cosine Similarity between the baseline and the original')
    plt.fill_between(temp_range, cs_list_lower, cs_list_upper, color='b', alpha=0.2, label='95% Confidence Interval')
    plt.ylim([0.0,1.1])
    plt.xlabel("SNR (dB)")
    ...
    plt.figure(figsize=(20, 6), dpi=300)
    plt.plot(temp_range, mi_list_mean, marker='o')
    # plt.axhline(y=mi_base_orig_mean, color='r', linestyle='--', label='Mutual Information between the baseline and the original')
    plt.fill_between(temp_range, mi_list_lower, mi_list_upper, color='b', alpha=0.2, label='95% Confidence Interval')
    plt.ylim([0.0,5.1])
    plt.xlabel("SNR (dB)")
